...
PollableSocket::PollableSocket(SocketType Type, string ip, uint16 port,  int (*Handler)(PollableSocket&))
{
...
PollableSocket::PollableSocket(SocketType Type, int fd, int (*Handler)(PollableSocket&))
{
	this->Type = Type;
...
size_t PollableSocket::Read(uint8 *Buffer, size_t size)
{
...
int PollableSocket::Close()
{
	return close(FileDescriptor);
}

int PollableSocket::AddToMessage(uint8* Buffer, size_t Size, struct WebSocketHeader Header)
{
